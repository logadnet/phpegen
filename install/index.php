...
$script = "PhETpl";
$ServerErrors = array();
$config_file_name = '../core/config.php';
function getBaseUrl() {
...
function check_($code) {
    $siteurl = urlencode(getBaseUrl());
    $curl = curl_init();
	curl_setopt_array($curl, array(
	    CURLOPT_URL => 'http://verify.logad.net/api/validate/licensekey/'.$code.'/2',
...
function check_domain(){
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'http://verify.logad.net/api/validate/host/'.$_SERVER['SERVER_NAME'].'/2',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_CUSTOMREQUEST => "POST",
        CURLOPT_SSL_VERIFYPEER => false
    ));
    $check = json_decode(curl_exec($curl), 1);
    if ($check['status'] != 200) {
        $html = "You don't have permission to use this product. The message from server is: ".$check['message']." <br > Contact the product developer";
        die($html);
    }
}
if (!empty($_POST['install'])) {
    ...
    $checkk = check_(trim($_POST['purshase_code']));
    if(!empty($checkk)) {
        if ($checkk['status'] != 200) {
            $ServerErrors[] = $checkk['message'];
        }
    }
    else{
...
// +------------------------------------------------------------------------+
// | '.$script.'
// | Copyright (c) '.date("Y").' HENT Technologies. All rights reserved.
// +------------------------------------------------------------------------+

	//Database credentials
	$dbhost		=	"'. $_POST['sql_host'] .'";	/* DB host*/
	$dbuser		=	"'. $_POST['sql_user'] .'";			/* DB user*/
	$dbpass		=	"'. $_POST['sql_pass'] .'";				/* DB pass*/
	$dbname		=	"'. $_POST['sql_name'] .'";			/*Database name*/
...
';
        ...
        if ($config_file) {
            ...
            if ($query === true) {
                $verified = false;
                $checker = check_success(trim($_POST['purshase_code']));
                if(!empty($checker)) {
                ...
                if($verified === true){
                    ...
                    $site_auth = $checker['site_auth'];
                    $sitename = mysqli_real_escape_string($con, $_POST['siteName']);
                    $siteEmail = mysqli_real_escape_string($con, $_POST['siteEmail']);
                    
                    $stmt = $con->prepare("UPDATE `site_settings` SET `site_auth` = ?, `site_name` = ?, `site_email` = ?");
                    $stmt->bind_param("sss", $site_auth, $sitename, $siteEmail);
                    $stmt->execute();

                    if($stmt->affected_rows == 1){
                        $admin_username = mysqli_real_escape_string($con, $_POST['admin_username']);
                        $admin_password = mysqli_real_escape_string($con, password_hash($_POST['admin_password'],PASSWORD_DEFAULT));
                        $admin_email = $siteEmail;
    
                        $query = "INSERT INTO `users` (`username`,`password`, `email`, `doj`, `type`, `email_verified`) VALUES ('$admin_username', '$admin_password', '$admin_email', '".time()."', 'admin', 'yes')";
                        
                        $time = time();
                        $admin = "admin";
                        $yes = "yes";
                        
                         $stmt = $con->prepare("INSERT INTO `users` (`username`,`password`, `email`, `doj`, `type`, `email_verified`) VALUES (?, ?, ?, ?, ?, ?)");
                        $stmt->bind_param("sssiss", $admin_username, $admin_password, $admin_email, $time, $admin, $yes);
                        $stmt->execute();
                        
                        if($stmt->affected_rows == 1){
                            if(mysqli_error($con)){
                                 $ServerErrors[] = "Error in installation: ".mysqli_error($con);
                            }
                            else{
                                $success = 'Script successfully installed, don"t forget to delete the install folder. Please wait ..';
                            }
                        }
                        else{
                            $ServerErrors[] = "Error in installation: ".mysqli_error($con);
                    ...
                    $query = "DROP DATABASE ".$_POST['sql_name'];
                    $run = mysqli_query($con,$query);
                    $ServerErrors[] = "Your purchase code couldn't be verified.";
                }
            } else {
...
<head>
    ...
    <link rel="shortcut icon" type="image/png" href="../uploads/logo/icon.png"/>
                ...
                <div>
                    ...
                    <div class="setting-well">
                        ...
                        <?php } else if ($page == 'req') { ?>
                            <div class="req">
                                <table class="table table-hover">
                                    ...
                                    <tr>
                                        <td>database.sql</td>
                                        <td>Required database.sql for the installation </td>
                                ...
                                <form action="?page=installation" method="post">
                                    ...
                                    if($disabled === true){
                                        ?>
                                    <button type="button" class="btn btn-main" onclick="window.location.reload()">Reload</button>
                                        <?php
                                    } else {
                                        ?>
                        ...
                        <?php } else if ($page == 'finish') { ?>
                            <div class="req">
                                <p>Congratulations! <?=$script?> has been successfully installed and your website is ready.</p>
                                <p>Login to your admin panel to make changes and modify any default content according to your needs.</p>
                                <br>
                                <p>If you have any question, please <a href="mailto:logadscripts@gmail.com" class="main">let us know</a>.</p>
                                <br><br>
                                <a href="../admin/login" class="btn btn-main" style="line-height: 50px;">Let's Start !</a>
                            </div>
                                ...
                                <form action="?page=installation" method="post" class="form-horizontal install-site-setting">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <span class="help-block">Enter your product purchase code</span>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="col-md-1"></div>
